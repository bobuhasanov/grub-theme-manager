...
[![Platform](https://img.shields.io/badge/Platform-Linux%20Mint%20%7C%20Ubuntu%20%7C%20Debian-blue.svg)]()
...
Designed specifically for dual-boot setups (Linux Mint / Ubuntu + Windows 11 / 10), fixing early boot ACPI BIOS error codes and providing an aesthetic, seamless boot experience.
...
* 🎮 **1:1 Live Interactive Simulator:**
  ...
  * Live countdown timer with pause and reset controls.
* 💎 **macOS Obsidian GRUB 2 Theme:**
  ...
  * **Official OS Icons:** Clean icons for Linux Mint, Windows 11, UEFI Firmware, and Ubuntu.
* 🤫 **Silent Boot Engineering:**
  * Suppresses noisy early ACPI BIOS error messages via `/etc/sysctl.d/10-console-messages.conf` (`kernel.printk = 3 4 1 3`).
  * Enables clean, silent systemd boot parameters (`quiet splash loglevel=3 vt.global_cursor_default=0 systemd.show_status=0`).
  * Integrates Early KMS (`i915`) for instant visual handover to your display driver.
...
1. Verifies Python 3, GTK3, Cairo, and GObject Introspection packages.
2. Installs the theme assets to `~/.local/share/grub-themes/mac-obsidian/`.
3. Installs the `grub-theme-manager` binary to `~/.local/bin/`.
4. Creates a desktop shortcut in your **Application Menu** (under Administration / Preferences).
...
2. Interact with the **Live Simulator**:
   ...
   * Use the **Boot Timeout** slider to adjust the countdown (e.g. 5s, 10s, 15s).
3. Click **"Save & Re-apply Settings"** to write all changes to `/boot/grub/themes/` and run `update-grub`.
...
* **Operating Systems:**
  ...
  * Debian 11 / 12
  * Pop!_OS, Zorin OS, Fedora (GRUB 2.04+)
* **Hardware & Resolution:**
  * 1920x1080 (16:9 Native FHD)
  * Compatible with ASUS Vivobook, Zenbook, Dell XPS, Lenovo ThinkPad, HP, and desktop monitors.
